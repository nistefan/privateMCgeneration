...
#                           #
###############           #
maxIdlingJobsAccepted = 4999 #600 #3000
numbergfalstart = 500
maxFilesToBeChecked   = 1000

SplitSizeStepOfFileList = 1 #2 #10

OldDirectoriesSuffix = "GENSIM" #"PremixStep1" #"AODSIM" #"2018-MINIAODSIM" #"2018-AODSIM" #"PremixStep1" #"GENSIM"

NewDirectoriesSuffix = "PremixStep1" #"AODSIM" #"PremixStep1"  #"2018-NANOAODSIM"
      #              ############
    #                           #
  #                  as needed  #
#                               #
...
while(countloops < 10):
  countloops += 1
  print(pycolors.underline+"This is loop number : "+str(countloops)+pycolors.end)
#############################################
...
################ This script assumes first schedd being your submission node; if it's not the case: change it.
  ...
  if args.submit:
    if getIdlingJobs > maxIdlingJobsAccepted:
        print(pycolors.blue+"Stop running this script due to too many jobs idling on HTCondor; try later again"+pycolors.end)
        break
    ...
    #      sys.exit("Stopping test")          
      ...
      if not os.path.exists("/pnfs/desy.de/cms/tier2/store/user/nstefano/"+ExtraDirectoryCreationOnPNFS+DirectoryName+"/"+outputRootfile):
       if not os.path.exists(NewDirectoryPath+"/"+outputRootfile): # and not os.path.exists("/pnfs/desy.de/cms/tier2/store/user/nstefano/"+ExtraDirectoryCreationOnPNFS+DirectoryName+"/"+outputRootfile):
        if not os.path.exists(NewDirectoryPath+"/"+TemplateFileNameAdjusted) and args.submit:
          numberOfFiles += 1
 ...
 #           if FileName in item_ask_condor:
      ...
      if args.check:
          ...
          if os.path.exists(NewDirectoryPath+"/"+outputRootfile): # or os.path.exists("/pnfs/desy.de/cms/tier2/store/user/nstefano/"+ExtraDirectoryCreationOnPNFS+DirectoryName+"/"+outputRootfile):
              ...
              jobInBatch = False
              filesChecked += 1
              print(pycolors.green+"filesChecked is "+str(filesChecked)+pycolors.end)
              if filesChecked > maxFilesToBeChecked:
                  filesChecked = 0
                  print(pycolors.green+"Maximum number of files is checked. Stop running."+pycolors.end)
                  break
                  #sys.exit(pycolors.green+"Maximum number of files is checked. Stop running."+pycolors.end)
              ...
              if not jobInBatch:
                     ...
                     # if FileName in checkifAlreadyBeenCheckedInPreviousRun: getNEventsTier2 = 200
                      ...
                      if getNEventsTier2 == 200:
                           ...
                           if not os.path.exists("/pnfs/desy.de/cms/tier2/store/user/nstefano/"+ExtraDirectoryCreationOnPNFS+DirectoryName+"/"+outputRootfile) and numbergfalstart < filesChecked:
#os.path.exists("/pnfs/desy.de/cms/tier2/store/user/nstefano/"+ExtraDirectoryCreationOnPNFS+DirectoryName+"/"+outputRootfile): # or  os.path.exists("/pnfs/desy.de/cms/tier2/store/user/nstefano/"+DirectoryName+"/"+outputRootfile):                                             
                               ...
                               getoutputcopy = subprocess.Popen([gfalCopyCommand], shell=True,
                                             stdout=subprocess.PIPE).communicate()[0]
                               #sys.exit()
                           ...
                           if os.path.exists(NewDirectoryPath+"/"+outputRootfile) and os.path.exists("/pnfs/desy.de/cms/tier2/store/user/nstefano/"+ExtraDirectoryCreationOnPNFS+DirectoryName+"/"+outputRootfile):
                               ...
                               print(pycolors.green+"os.path.exists(/pnfs/desy.de/cms/tier2/store/user/nstefano/"+ExtraDirectoryCreationOnPNFS+DirectoryName+"/"+rootOutputFile+")  is "+str(os.path.exists("/pnfs/desy.de/cms/tier2/store/user/nstefano/"+ExtraDirectoryCreationOnPNFS+DirectoryName+"/"+rootOutputFile))+pycolors.end)
